...
exports.createUser = [
    ...
    check('password').exists().trim().isLength({ min: 4 }).withMessage('Password must be at least 4 characters'),
    check('fullName')
        .exists()
        .trim()
        .isLength({ min: 4 })
    ...
    async (req, res, next) => {
        try {
            ...
            const user = new User({
                ...
                profilePicture:
                    'https://res.cloudinary.com/de2ymful4/image/upload/v1654559614/assets/stock_ehxcl9_idaplu.png',
            ...
            console.log(errors);
            return res.status(400).json({ message: 'Error creating new account', errors });
        }
    },
    ...
    async (req, res) => {
        try {
            ...
            const {
                data: { token },
            } = await axios.post(`${process.env.API_SERVER_URL}/login`, res.locals.user);

            return res.json({ token });
        } catch (errors) {
            return res.status(400).json({ message: 'Error retrieving authentication token', errors });
        }
    },
];
...
exports.getUser = [
    ...
    async (req, res) => {
        try {
            ...
            if (user == null) {
                return res.status(401).json({ message: 'Invalid token, no user exists with this token' });
            } else {
                return res.json({ user });
            }
        } catch (errors) {
            return res.status(401).json({ message: 'Error getting user information', errors });
        }
    },
];
...
exports.updateUser = [
    ...
    async (req, res, next) => {
        try {
            ...
            return res.status(500).json({ message: 'Error occurred while updating user!', error });
        }
    },
];
...
exports.updateBookmarked = [
    ...
    async (req, res, next) => {
        ...
        if (typeof cryptoName === undefined) return res.status(400).json({ message: 'Crypto name not provided' });
        ...
        try {
            ...
            return res.status(500).json({ message: 'Error updating bookmarks', errors });
        }
    },
];
